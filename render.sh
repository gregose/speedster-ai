...
# Headless rendering: --render --backend=Manifold doesn't need a display
# (unlike --preview which uses OpenGL and requires xvfb in containers)
...
$OPENSCAD "$SCAD" --render --backend=Manifold --camera=100,800,50,$CENTER    --imgsize=$SIZE -o "$OUTDIR/front.png"               2>/dev/null &
$OPENSCAD "$SCAD" --render --backend=Manifold --camera=-100,-800,50,$CENTER  --imgsize=$SIZE -o "$OUTDIR/back.png"                2>/dev/null &
